...
import ChatbotContent from "./Content/ChatbotContent"; // Ini sekarang hanya akan berisi bubble chat
...
import { HiOutlinePencilAlt } from "react-icons/hi"; // Untuk ikon New Chat
import { HiMenuAlt3 } from "react-icons/hi"; // Untuk ikon hamburger menu
...
const HomePage = () => {
    ...
    const [activeTab, setActiveTab] = useState("chatbot"); // Set default ke chatbot agar langsung terlihat
    ...
    // Definisikan tinggi header dan footer sebagai variabel untuk Tailwind
    // Sesuaikan dengan yang Anda gunakan di ChatbotContent
    const HEADER_HEIGHT_CLASS = "h-16"; // Misal header 64px
    const FOOTER_HEIGHT_CLASS = "h-20"; // Misal footer 80px
    const HEADER_PADDING_TOP = "pt-16"; // padding yang sama dengan tinggi header
    const FOOTER_PADDING_BOTTOM = "pb-20"; // padding yang sama dengan tinggi footer


    // --- Render Section ---
    return (
        // Container utama aplikasi: flex-col, mengisi seluruh layar
        <div className="flex flex-col w-full h-screen bg-[#000814] font-sans">
            {/* Mobile Hamburger Button - Posisinya tetap fixed di atas konten */}
            <button
            ...
            <div
                className={`
                    ...
                    transition-all duration-300 ease-in-out px-4 w-64
            ...
            {/* Main Content Area - Right Section (akan menampung chatbot atau konten lainnya) */}
            <div className="flex-grow bg-[#000814] flex flex-col overflow-hidden text-white">
                {/* Konten akan memiliki padding-top dan padding-bottom yang dinamis */}
                {/* Bagian ini akan memiliki padding horizontal yang sesuai dengan desain */}
                {/* Hapus lg:p-5 rounded-lg shadow-xl dari sini */}

                {/* Header Global (fixed) - Hanya tampil jika activeTab adalah 'chatbot' */}
                {activeTab === "chatbot" && (
                    <div className={`fixed top-0 left-0 w-full bg-[#001D3D] flex items-center justify-between shadow-md px-4 py-3 sm:px-6 ${HEADER_HEIGHT_CLASS} z-10 md:pl-[16rem]`}> {/* pl untuk mengakomodasi sidebar desktop */}
                        <div className="flex items-center gap-3">
                            {/* Pastikan ikon menu hamburger ada */}
                            <button onClick={toggleMobileSidebar} className="md:hidden"> {/* Tambah tombol hamburger untuk mobile */}
                                <HiMenuAlt3 className="w-6 h-6 text-white cursor-pointer" />
                            </button>
                            {/* Jika Anda ingin ikon hamburger di desktop (sidebar selalu terlihat), jangan pakai md:hidden */}
                            <h2 className="text-lg sm:text-xl font-semibold text-white">
                                Chat dengan Bot
                            </h2>
                        </div>
                        <button
                            onClick={startNewConversation} // Gunakan startNewConversation dari useChatbot
                ...
                <div className="flex-1 overflow-y-auto"
                     style={
                        activeTab === 'chatbot' ?
                        { paddingTop: `var(--header-height)`, paddingBottom: `var(--footer-height)` } :
                        {} // Tidak ada padding khusus jika bukan tab chatbot
                    }>
                    {activeTab === "home" && (
                        <div className="flex flex-col justify-center items-center h-full text-center animate-fade-in px-4 py-4"> {/* Tambah padding */}
                    ...
                    {activeTab === "chatbot" && (
                        // ChatbotContent sekarang hanya menerima pesan dan handler internal
                        <ChatbotContent
                            loading={isLoading}
                            ...
                            // onNewChat dan isBotTyping tidak perlu lagi di sini karena sudah di handle di Homepage
                            isBotTyping={isBotTyping} // Tetap kirim isBotTyping jika ingin indikator typing di dalam ChatbotContent
                            // Hapus onNewChat karena sudah di header global
                        />
                    )}

                    {activeTab === "learn" && (
                        <div className="px-4 py-4"> {/* Tambah padding */}
                            {showDetailLatihan ? (
                                <DetailHasilLatihan
                ...
                {/* Footer Global (fixed) - Hanya tampil jika activeTab adalah 'chatbot' */}
                {activeTab === "chatbot" && (
                    <div className={`fixed bottom-0 left-0 w-full bg-[#001D3D] shadow-inner px-4 py-3 sm:px-6 ${FOOTER_HEIGHT_CLASS} z-10 md:pl-[16rem]`}> {/* pl untuk mengakomodasi sidebar desktop */}
